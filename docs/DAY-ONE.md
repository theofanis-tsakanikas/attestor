...
## Reading the lake from the console is operator work, not the system's

Lake Formation governs this account's catalogue — an account-wide setting this project did not
make and does not own — so an IAM policy alone does not open a table. The layers grant exactly
three principals what they need: `attestor-github-deploy`, `attestor-tools`, `attestor-runtime`.

A person opening Athena is none of them, and Lake Formation admin is not a substitute: it
confers the right to grant, not the right to read. The query comes back

    COLUMN_NOT_FOUND: Column 'tenant_id' cannot be resolved or requester is not authorized

which reads like a schema fault and is an authorisation one.

**This does not belong in Terraform**, and the reasoning is worth keeping because the opposite
was tried first. A human's console access is not a component of the system: Attestor runs
identically without it, the grant would be tied to the estate's lifecycle and recreated on
every deploy, and a personal ARN committed to a public repository means nothing to anyone who
clones it. The layers describe the system. This describes who happens to be looking.

So it is done by hand, once, per person, and recorded here — the same treatment as the model
access request and the IdP registration:

```
principal=arn:aws:iam::<account>:user/<name>
for db in attestor_gold attestor_gold_ref; do
  aws lakeformation grant-permissions --principal "DataLakePrincipalIdentifier=$principal" \
...
Granted to `fanis-admin` on 11 August 2026. It survives a teardown, because Lake Formation
grants outlive the tables they name — so this is done once per person, not once per estate.

Note the wildcard works here and does not work from the deploy role. A `TableWildcard` grant
requires the grantor to hold permission on the wildcard itself; a Lake Formation admin does,
and the role that merely created the tables does not. That asymmetry is why `infra/agent`
grants table by table.

And it is read-only on synthetic data. Every figure in this lake is generated by
`pipelines/seed/generate.py`; there is nothing here to be careful with except the habit.
